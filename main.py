...
def get_ids_data(): # Вспомогательная функция поиска ID
...
def get_phones_data(): # Вспомогательная функция для поиска Телефона
    with conn.cursor() as cur:
...
def data_change(client_id, client_name=None, client_surname=None, client_email=None,
                phone_number=None):  # Изменить данные сущ. клиента
    ids_res = get_ids_data()
    with conn.cursor() as cur:
        if client_id in ids_res:
